...
if(isset($_GET['action'])){
		...
		$number=$_POST['number'];
		if($_GET['type']=='asc'){
			$query="update orderdetail set number=number+1, sumprice=number*price where orderid=$con1 and productid=$con2";
...
<form method="post">	
	...
	<table class="table table-bordered brandadmin">
		...
		<tr style="vertical-align:middle;">
			...
			<td width="25%"><img src="../image/<?=$item['productImage']?>"width="50%"></td>
			<td><?=number_format($item['productPrice'],0,',','.')?> đ</td>
			<td><input style="display: <?=$order['status']==3?'none':''?>;" <?=$item['number']==0?'disabled':''?> type="button" value="-" onclick="location='?option=orderdetail&id=<?=$_GET['id']?>&action=update&type=desc&id=<?=$item['orderid']?>&productid=<?=$item['productid']?>';"> <?=$item['number']?><input style="display: <?=$order['status']==3?'none':''?>;" type="button" value="+" onclick="location='?option=orderdetail&id=<?=$_GET['id']?>&action=update&type=asc&id=<?=$item['orderid']?>&productid=<?=$item['productid']?>';"> <br><br></td>
			<td><?=number_format($item['productPrice']*$item['number'],0,',','.')?> đ</td>
		</tr>
		<?php endforeach; ?>
		</tbody>
	</table>
	<h2>TRẠNG THÁI ĐƠN HÀNG</h2>
		...
		<section><input <?=$order['status']==3?'disabled':''?> type="submit" value="Update đơn hàng" class="btn btn-primary">&emsp;&emsp;<input type="button" value="Quay lại" class="btn btn-primary" onclick="location= '?option=order&status=<?=$order['status']?>';"></section>
</form>